...
function fitzstudio_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'fitzstudio'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'fitzstudio' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'fitzstudio' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'fitzstudio' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'fitzstudio_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);
	// Add the controls
	$wp_customize->add_control(	'fitzstudio_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'fitzstudio' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'fitzstudio' ),
				'full-content'	=> __( 'Full content', 'fitzstudio' )
			),
...
add_action( 'customize_register', 'fitzstudio_customize_register' );
/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function fitzstudio_customize_preview_js() {
	wp_enqueue_script( 'fitzstudio_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'fitzstudio_customize_preview_js' );
/**
 ...
 */
function fitzstudio_sanitize_length( $value ) {
    if ( ! in_array( $value, array( 'excerpt', 'full-content' ) ) ) {
        $value = 'excerpt';
	}
    return $value;
}
if ( ! function_exists( 'fitzstudio_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see fitzstudio_custom_header_setup().
 */
function fitzstudio_header_style() {
